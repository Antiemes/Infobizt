...
\begin{frame}{Folyamtitkosítási (CFB) mód}
  ...
  \begin{center}
    \begin{tikzpicture}
      ...
      \node[right = of e] (dot2) {};
      \draw[thick] (dot)-|(dot2.center);
      \draw[->,thick] (dot2.center)--(e);
...
\begin{frame}{Számláló (CTR) mód}
  ...
  \begin{center}
    \begin{tikzpicture}%[transform canvas={scale=0.6}]
      \node[draw,rectangle] (e) {$E$};
      \node[above = .3 cm of e] (iv) {$IV + n$};
      \draw[->,thick] (iv)--(e);

      \node[draw,circle,below = .3 cm of e] (xor) {+};
      \draw[->,thick] (e)--(xor);
      
      \node[left = of e] (k) {$K$};
      \draw[->,thick] (k)--(e);

      \node[left = of xor] (p) {$P_n$};
      \draw[->,thick] (p)--(xor);

      \node[right = of xor] (c) {$C_n$};
      \draw[->,thick] (xor)--(c);
      
      \node[above,font=\bfseries] at (current bounding box.north) {A titkosítás menete};
    \end{tikzpicture}
    \begin{tikzpicture}%[transform canvas={scale=0.6}]
      \node[draw,rectangle] (e) {$E$};
      \node[above = .3 cm of e] (iv) {$IV + n$};
      ...
      \node[left = of xor] (c) {$C_n$};
      \draw[->,thick] (c)--(xor);

      \node[right = of xor] (p) {$P_n$};
      \draw[->,thick] (xor)--(p);
      
      \node[above,font=\bfseries] at (current bounding box.north) {A visszafejtés menete};
    \end{tikzpicture}
  \end{center}
